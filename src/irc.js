...
function Client(server, nick, opt) {
  ...
  if (typeof arguments[2] == "object") {
    ...
    for (var i = 0; i < keys.length; i++) {
      var k = keys[i];
      if (arguments[2][k] !== undefined) self.opt[k] = arguments[2][k];
    }
  }
    ...
    switch (message.command) {
      ...
      case "err_umodeunknownflag":
        if (self.opt.showErrors)
          util.log(
            "\u001b[01;31mERROR: " + util.inspect(message) + "\u001b[0m"
          );
        break;
...
Client.prototype.connect = function (retryCount, callback) {
  ...
  if (self.opt.secure) {
    ...
    self.conn = net.createConnection(
      connectionOpts,
  ...
  self.conn.addListener("end", function () {
    if (self.opt.debug) util.log('Connection got "end" event');
  });
  self.conn.addListener("close", function () {
    if (self.opt.debug) util.log('Connection got "close" event');

    if (self.conn && self.conn.requestedDisconnect) return;
    if (self.opt.debug) util.log("Disconnected: reconnecting");
    if (self.opt.retryCount !== null && retryCount >= self.opt.retryCount) {
      if (self.opt.debug) {
        util.log(
          "Maximum retry count (" + self.opt.retryCount + ") reached. Aborting"
        );
      }
